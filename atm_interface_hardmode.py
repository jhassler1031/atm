...
def ui_withdrawel(db, balance):
    while True:
        ...
        if amount < balance:
            ...
            print("Insufficient Funds.  Would you like to ")
            print("1) Use overdraft protection ($15 fee)")
            print("2) Enter a different amount")
            print("Press <enter> to exit.")
            choice = input("> ")
            ...
            if choice == "1":
                withdrawel(db, amount)
                balance -= (amount + 15)
                print(f"You have withdrawn ${amount} and the $15 fee has been applied.")
                break
            elif choice == "":
